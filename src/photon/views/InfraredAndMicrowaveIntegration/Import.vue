<template>
  <div v-loading="loading">
    <breadcrumb :pathArr="local"></breadcrumb>
    <el-form label-width="150px">
      <el-row :gutter="20" type="flex" justify="center">
        <el-card class="padding_in">
          <el-col :span="11" class="margin">
            <el-form-item label="入射角(°)">
              <el-input-number
                v-model="form.lightAngle"
                :min="0"
                :max="180"
                :precision="2"
              ></el-input-number>
            </el-form-item>
          </el-col>
          <el-col :span="11" class="margin">
            <el-form-item label="基底介电常数">
              <el-input
                v-model="form.baseElectric"
                placeholder="请输入基底介电常数"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11" class="margin">
            <el-form-item label="基底红外发射率">
              <el-input
                v-model="form.baseInfraredEmissivity"
                placeholder="请输入基底红外发射率"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11" class="margin">
            <el-form-item label="基底材料">
              <el-input
                v-model="form.baseMaterial"
                placeholder="请输入基底材料"
              ></el-input>
            </el-form-item>
          </el-col>
          <!-- <el-col :span="11" class="margin">
            <el-form-item label="基底厚度">
              <el-input
                v-model="form.baseThickness"
                placeholder="请输入基底厚度"
              ></el-input>
            </el-form-item>
          </el-col> -->
          <el-col :span="11" class="margin">
            <el-form-item label="金属贴片红外发射率">
              <el-input
                v-model="form.patchInfraredEmissivity"
                placeholder="请输入金属贴片红外发射率"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11" class="margin">
            <el-form-item label="金属贴片材料">
              <el-input
                v-model="form.patchMaterial"
                placeholder="请输入金属贴片材料"
              ></el-input>
            </el-form-item>
          </el-col>
          <!-- <el-col :span="11" class="margin">
            <el-form-item label="金属贴片厚度">
              <el-input
                v-model="form.patchThickness"
                placeholder="请输入金属贴片厚度"
              ></el-input>
            </el-form-item>
          </el-col> -->
          <el-col :span="11" class="margin">
            <el-form-item label="数据来源">
              <el-select v-model="form.dataFrom" placeholder="请选择">
                <el-option
                  v-for="item in options.dataFrom"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="11" class="margin">
            <el-form-item label="数据">
              <file-input
                v-model="form.data"
                tips="请选择相应文件"
              ></file-input>
              <!-- <file-upload v-model="form.data" :files="form.data"></file-upload> -->
            </el-form-item>
          </el-col>
          <el-col :span="11" class="margin">
            <el-form-item label="程序">
              <file-input
                v-model="form.process"
                tips="请选择相应文件"
              ></file-input>
            </el-form-item>
          </el-col>
          <el-col :span="11" class="margin">
            <el-form-item label="结构">
              <file-input
                v-model="form.struct"
                tips="请选择相应文件"
              ></file-input>
            </el-form-item>
          </el-col>
          <el-col :span="24" class="margin">
            <el-form-item>
              <el-button type="primary" @click="handleSubmitClick">
                保存
              </el-button>
            </el-form-item>
          </el-col>
        </el-card>
      </el-row>
    </el-form>
  </div>
</template>

<script>
/**
 * 红外微波一体化导入组件
 * @author Chris
 * */
import AntennaTypes from "../../components/config/AntennaTypes";
import FileUpload from "../../../components/form/input/FileUpload.vue";
import FileInput from "../../../components/form/input/FileInput.vue";
import breadcrumb from "../../../components/navigation/PageBreadcrumb";
export default {
  name: "NanoAntennaImportForm",
  components: {
    FileUpload,
    FileInput,
    breadcrumb,
  },
  data() {
    return {
      loading: false,
      options: AntennaTypes,
      form: {
        baseElectric: "", //基底介电常数
        baseInfraredEmissivity: "", //基底红外发射率
        baseMaterial: "", //基底材料
        baseThickness: "", //基底厚度
        dataFrom: "", //数据来源
        lightAngle: "", //入射角
        patchInfraredEmissivity: "", //金属贴片红外发射率
        patchMaterial: "", //金属贴片材料
        patchThickness: "", //金属贴片厚度
        data: [],
        process: [],
        struct: [],
      },
      local: [
        {
          name: "首页",
          url: "/photonm",
        },
        {
          name: "红外宽带高反及微波宽带高透",
          url: "/photonm/InfraredAndMicrowaveIntegration",
        },
        {
          name: "导入",
          url: "",
        },
      ],
    };
  },
  methods: {
    /**
     * 构建上传数据
     * */

    buildCreateForm() {
      const param = new FormData();
      param.append("baseElectric", this.form.baseElectric);
      param.append("baseInfraredEmissivity", this.form.baseInfraredEmissivity);
      param.append("baseMaterial", this.form.baseMaterial);
      param.append("baseThickness", this.form.baseThickness);
      param.append("dataFrom", this.form.dataFrom);
      param.append("lightAngle", this.form.lightAngle);
      param.append(
        "patchInfraredEmissivity",
        this.form.patchInfraredEmissivity
      );
      param.append("patchMaterial", this.form.patchMaterial);
      param.append("patchThickness", this.form.patchThickness);
      param.append("data", this.form.data[0].raw);
      param.append("process", this.form.process[0].raw);
      param.append("struct", this.form.struct[0].raw);
      return param;
    },

    /**
     * 发送请求
     * */

    submitForm() {
      this.loading = true;

      if (this.$route.query.data == "0") {
        this.$postserver.postFile(
          "/infrared_projection/save",
          this.buildCreateForm(),
          function (response) {
            if (response.meta.code == "0") {
              this.loading = false;
              this.$alert("上传成功,点击确定返回上一页", "成功", {
                confirmButtonText: "确定",
                callback: (action) => {
                  this.$router.push({
                    path: "MicrowaveTransmission",
                  });
                },
              });
            } else {
              this.loading = false;
              this.$alert("上传失败，请点击重试", "上传失败", {
                confirmButtonText: "确定",
                callback: (action) => {},
              });
            }
          }.bind(this)
        );
      } else if (this.$route.query.data == "1") {
        this.$postserver.postFile(
          "/infrared_projection/import",
          this.buildCreateForm(),
          function (response) {
            if (response.meta.code == "0") {
              this.loading = false;
              this.$alert("上传成功,点击确定返回上一页", "成功", {
                confirmButtonText: "确定",
                callback: (action) => {
                  this.$router.push({
                    path: "InfraredAndMicrowaveIntegration",
                  });
                },
              });
            } else {
              this.loading = false;
              this.$alert("上传失败，请点击重试", "上传失败", {
                confirmButtonText: "确定",
                callback: (action) => {},
              });
            }
          }.bind(this)
        );
      } else {
      }
    },

    /**
     * 点击保存按钮
     * */

    handleSubmitClick() {
      this.submitForm();
    },
  },
};
</script>

<style lang="scss">
</style>